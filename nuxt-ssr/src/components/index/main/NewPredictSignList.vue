<template>
  <!-- <transition name="opacity"> -->
    <div class="newPredictSignList_wrap">
      <div class="newPredictSignList_inner" v-if="items.length>0">
        <div class="title">
          <ul>
            ...
            <li class="earning">
              <span @click="sortingItems('targetEarningRateText')">목표수익률
                <i class="xi-caret-down" :class="{on: sortFlag['targetEarningRateText'] === true, off: sortFlag['targetEarningRateText'] === false}"></i>
              </span>
            </li>
            <li class="validate">
...
<script>
import NewPredictSign from '@/src/components/index/main/NewPredictSign';
export default {
  ...
  methods: {
    ...
    scrollSignList() {
      const $scroll_inner = this.$refs.scroll_inner;
      if($scroll_inner.scrollTop >= $scroll_inner.scrollHeight - $scroll_inner.offsetHeight - 2) {  //오차가 있는듯 -2
        this.shadow = false;
      } else {
        this.shadow = true;
      }
